...
public class LimeLight extends SubsystemBase {
    ...
    public tagId getObeliskID(){
        ...
        public LLResultTypes.FiducialResult getTargetInfo(){
            ...
            List<LLResultTypes.FiducialResult> results = detectedtags.getFiducialResults();
            if (results!=null && !results.isEmpty()){
                if(RobotContainer.isRedAlliance() && results.get(0).getFiducialId() == 24){
                    return results.get(0);
                } else if (RobotContainer.isRedAlliance() && results.get(1).getFiducialId() == 24) {
                    return results.get(1);
                }else if (!RobotContainer.isRedAlliance() && results.get(0).getFiducialId() == 20){
                    return  results.get(0);
                }else if (!RobotContainer.isRedAlliance() && results.get(1).getFiducialId() == 20){
                    return results.get(1);
                }else{
                    return  null;
                }

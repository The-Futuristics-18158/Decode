...
public class HuntMode extends SequentialCommandGroup {

    // constructor
    public HuntMode() {

        addCommands (
                //button tells if were looking for a purple ball or a green-
                //-(maybe should have a hunt green and a hunt purple command group?
                // definitely a grab whatever is closest too
                // allow manual drive to turn robot to the general direction of a ball
                // webcam looks for ball
                // turn to ball ... this used a PID control on the x offset (camera)
                // turn on intake
                // when detected drive to nearest ball
                // turn off intake when you have a ball
        );
    }
